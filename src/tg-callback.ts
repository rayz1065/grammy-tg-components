...
type TgCallbackContext = Context & TgCallbackFlavor;

type CallbackMiddleware<
  C extends TgCallbackContext,
  T extends unknown[],
> = Middleware<
...
type TgCallbackMatchResult<C extends TgCallbackContext, T extends unknown[]> = {
  match: TgCallback<C, T>;
  values: T;
} | null;

interface TgCallbackMatchable<
  C extends TgCallbackContext,
  T extends unknown[],
> {
...
> implements TgCallbackMatchable<C, T>
{
  ...
  /**
   * Get the callback data for this TgCallback, the passed values will be
   * encoded to JSON.
   */
  ...
  /**
   * Get an inline keyboard button for this callback with the specified text.
   */
  public getBtn(
    text: string,
    ...values: T
  ...
  /**
   * Checks if the passed callback matches this element, if the prefix matches
   * and the rest can be decoded returns the match, on failure returns `null`.
   */
  public match(cbData: string): TgCallbackMatchResult<C, T> {
 ...
 */
export class TgCallbacksBag<C extends TgCallbackContext>
  implements TgCallbackMatchable<C, unknown[]>
{
